...
const PurchaseReturnViewPage = () => {
  ...
  const { data: purchaseData, isLoading } = useQuery({
    queryKey: ["purchase-return", decryptedId],
    queryFn: () => purchaseReturnService.getById(decryptedId, token),
    enabled: !!decryptedId,
  });

  const handlePrintPdf = useReactToPrint({
    content: () => containerRef.current,
    documentTitle: "PurchaseReturn",
    pageStyle: `
  ...
  const handleSaveAsPdf = () => {
    ...
    html2pdf()
      ...
      .set({
        margin: 10,
        filename: `PurchaseReturn_${decryptedId}.pdf`,
  ...
  const totalPurchaseSubPiece = purchasesubData.reduce((sum, row) => sum + (row.purchase_sub_piece || 0), 0);
  const totalPurchaseSubBox = purchasesubData.reduce((sum, row) => sum + (row.purchase_sub_box || 0), 0);
  ...
  return (
    <Page>
      ...
      <div className="bg-white border border-black max-w-4xl mx-auto p-8" ref={containerRef}>
        <h2 className="text-center font-bold text-2xl mb-6">PURCHASE RETURN</h2>
        ...
        <div className="grid grid-cols-2 border border-black mb-6">
          <div className="border-r border-black">
            ...
            <div className="p-3"><span className="font-bold">Ref No:</span> {purchase.purchase_ref}</div>
          </div>
          <div>
            <div className="p-3 border-b border-black"><span className="font-bold">City:</span> {buyer.buyer_city}</div>
            <div className="p-3"><span className="font-bold">Date:</span> {moment(purchase.purchase_date).format("DD-MMM-YYYY")}</div>
          </div>
        </div>
          ...
          <tbody>
            {purchasesubData.map((row, index) => (
              <tr key={index}>
                ...
                {singlebranch === "Yes" && doublebranch === "Yes" ? (
                  <>
                    <td className="p-2 border border-black text-center">{row.purchase_sub_box}</td>
                    <td className="p-2 border border-black text-center">{row.purchase_sub_piece}</td>
                  </>
                ) : (
                  <td className="p-2 border border-black text-right">{row.purchase_sub_box}</td>
                )}
              </tr>
            ))}
            <tr className="bg-gray-100 font-bold">
              <td className="p-2 border border-black" colSpan={2}>TOTAL</td>
              {singlebranch === "Yes" && doublebranch === "Yes" ? (
                <>
                  <td className="p-2 border border-black text-center">{totalPurchaseSubBox}</td>
                  <td className="p-2 border border-black text-center">{totalPurchaseSubPiece}</td>
                </>
              ) : (
                <td className="p-2 border border-black text-right">{totalPurchaseSubBox}</td>
              )}
            </tr>
          </tbody>
        </table>
        ...
        <div className="mt-6 text-sm border border-black">
          {totalPurchaseWeight > 0 && <p className="p-2 border-b border-black"><span className="font-bold">WEIGHT:</span> {totalPurchaseWeight} KG</p>}
          <p className="p-2 border-b border-black"><span className="font-bold">VEHICLE:</span> {purchase.purchase_vehicle_no}</p>
