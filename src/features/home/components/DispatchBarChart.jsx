...
const DispatchBarChart = ({
  ...
  const datasets = [
    { label: "Purchase", data: data.map((d) => d.Purchase), backgroundColor: "#34D399", borderRadius: 4 },
    { label: "Dispatch", data: data.map((d) => d.Dispatch), backgroundColor: "#60A5FA", borderRadius: 4 },
    { label: "Purchase Return", data: data.map((d) => d.PurchaseReturn), backgroundColor: "#7B3FBF", borderRadius: 4 },
    { label: "Dispatch Return", data: data.map((d) => d.DispatchReturn), backgroundColor: "#F87171", borderRadius: 4 },
  ];
  ...
  const tooltipCallbacks = {
    label: function (context) {
      ...
      if (singlebranch === "Yes" && doublebranch === "Yes") {
        let boxKey = "", pcsKey = "";
        switch (datasetLabel) {
          case "Purchase": boxKey = "Purchased_boxes"; pcsKey = "Purchased_pcs"; break;
          case "Dispatch": boxKey = "Sold_boxes"; pcsKey = "Sold_pcs"; break;
          case "PurchaseReturn": boxKey = "PurchaseReturned_boxes"; pcsKey = "PurchaseReturned_pcs"; break;
          case "DispatchReturn": boxKey = "DispatchReturned_boxes"; pcsKey = "DispatchReturned_pcs"; break;
          default: boxKey = ""; pcsKey = "";
        }
        result = `${datasetLabel}: ${itemData[boxKey] ?? 0} boxes and ${itemData[pcsKey] ?? 0} pieces`;
      } else if (singlebranch === "Yes" && doublebranch === "No") {
        let boxKey = "";
        switch (datasetLabel) {
          case "Purchase": boxKey = "Purchased_boxes"; break;
          case "Dispatch": boxKey = "Sold_boxes"; break;
          case "PurchaseReturn": boxKey = "PurchaseReturned_boxes"; break;
          case "DispatchReturn": boxKey = "DispatchReturned_boxes"; break;
          default: boxKey = "";
        }
        ...
        let pcsKey = "";
        switch (datasetLabel) {
          case "Purchase": pcsKey = "Purchased_pcs"; break;
          case "Dispatch": pcsKey = "Sold_pcs"; break;
          case "PurchaseReturn": pcsKey = "PurchaseReturned_pcs"; break;
          case "DispatchReturn": pcsKey = "DispatchReturned_pcs"; break;
          default: pcsKey = "";
        }
  ...
  return (
    <Card className="p-0 border-none">
        ...
        <div className="flex flex-col md:flex-row items-center md:justify-between mb-4 space-y-2 md:space-y-0 md:space-x-4 ">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full md:w-auto ">
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                {years?.map((year) => (
                  <DropdownMenuItem key={year} onSelect={() => handleChange(year, selectedMonth)}>
                    {year}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                {months?.map((month, index) => {
                  const isDisabled = Number(selectedYear) === currentYear && index > currentMonthIndex;
                  return (
                    <DropdownMenuItem key={month} disabled={isDisabled} onSelect={() => handleChange(selectedYear, month)}>
                      {month}
                    </DropdownMenuItem>
                  );
                })}
      ...
      <CardContent className="p-1 md:p-0">
        {isLoadingdashboord ? <Loader /> : isErrordashboord ? (
          ...
          <Bar
            ...
            options={{
              ...
              scales: {
                y: { beginAtZero: true, max: maxY + maxY * 0.1, ticks: { stepSize: Math.ceil((maxY + maxY * 0.1) / 5) } },
