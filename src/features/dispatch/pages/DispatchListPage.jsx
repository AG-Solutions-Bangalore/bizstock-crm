...
const DispatchListPage = () => {
  ...
  const { useDispatches, deleteDispatch } = useDispatch();
  const { data: dispatch, isLoading, isError, refetch } = useDispatches();
  ...
  const onWhatsApp = async (encryptedId) => {
    try {
      const data = await queryClient.fetchQuery({
        queryKey: ["dispatchByid", encryptedId],
        queryFn: () => fetchDispatchById(encryptedId, token),
      });
