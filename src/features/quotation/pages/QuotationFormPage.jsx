...
const QuotationFormPage = () => {
  ...
  const { useQuotationById, createQuotation, updateQuotation } = useQuotation(token);
  ...
  useEffect(() => {
    if (decryptedId && quotationByid?.quotation) {
      setFormData({
        ...
        quotation_vehicle_no: quotationByid.quotation.quotation_vehicle_no || "",
  ...
  const fetchAndSetStock = async (rowIndex, itemId, godownId) => {
    ...
    try {
      ...
      const opening = (safeNum(buyer?.openpurch) * itemPiece + safeNum(buyer?.openpurch_piece)) - 
                    (safeNum(buyer?.closesale) * itemPiece + safeNum(buyer?.closesale_piece)) -
                    (safeNum(buyer?.openpurchR) * itemPiece + safeNum(buyer?.openpurchR_piece)) +
                    (safeNum(buyer?.closesaleR) * itemPiece + safeNum(buyer?.closesaleR_piece));

      const current = (safeNum(buyer?.purch) * itemPiece + safeNum(buyer?.purch_piece)) -
  ...
  const handleTableChange = (value, rowIndex, fieldName) => {
    ...
    if (fieldName === "quotation_sub_item_id" || fieldName === "quotation_sub_godown_id") {
      fetchAndSetStock(rowIndex, updatedData[rowIndex].quotation_sub_item_id, updatedData[rowIndex].quotation_sub_godown_id);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if (decryptedId) {
        res = await updateQuotation.mutateAsync({ id: decryptedId, data: payload });
      } else {
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-4 mb-6 p-4">
        ...
        <h1 className="text-2xl font-bold">{decryptedId ? "Edit Quotation" : "Create Quotation"}</h1>
      </div>
      ...
      <div className="hidden md:block p-4">
        <form onSubmit={handleSubmit} className="space-y-6">
          <QuotationFormHeader 
            formData={formData} 
            handleInputChange={(e, f) => setFormData(p => ({ ...p, [f]: e?.target ? e.target.value : e }))} 
            isEdit={!!decryptedId} 
          />
          <QuotationFormTable 
            invoiceData={invoiceData} 
            handleTableChange={handleTableChange}
            addRow={() => setInvoiceData(p => [...p, { quotation_sub_item_id: "", quotation_sub_godown_id: "", quotation_sub_batch_no: "", quotation_sub_rate: "", quotation_sub_box: 0, quotation_sub_piece: 0, stockData: { total: 0, total_box: 0, total_piece: 0 } }])}
            removeRow={(i) => setInvoiceData(p => p.filter((_, idx) => idx !== i))}
          ...
          <div className="flex justify-end gap-2">
            <Button type="button" variant="outline" onClick={() => navigate("/quotation")}>Cancel</Button>
